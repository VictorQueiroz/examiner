...
describe('Validator', function() {
    ...
    beforeEach(function() {
        ...
        validator.loadFromJSON(require('../src/lang/en/messages'));
    });
    ...
    describe('validate()', function() {
        ...
        it('should automatically create "rules" property if none is defined while using only presets', function() {
            ...
            return validator.validate(null).catch(() => {
                assert.deepEqual(validator.errors, {
                    a1: {
                        required: true
                    },
        ...
        it('should support async rules', function() {
            ...
            return validator.validate({ test: 100 }).catch(function() {
                assert.deepEqual(validator.messages, {
                    test: {
        ...
        it('should always return a promise', function() {
            validator.update({
                rules: {
                    test: 'min:1|required|number'
                }
            });
            return validator.validate({test: 1}).then((result) => {
            });
        });

        it('should return a rejected promise when a error is encountered', function() {
            validator.update({
                rules: {
                    test: 'min:2'
                }
            ...
            return validator.validate({test: 1}).catch(() => {
                assert.deepEqual(validator.errors, {
                    test: {
                        min: true
                    }
        ...
        it('should return a resolved promise if no error is encountered', function() {
            validator.update({
                rules: {
                    test: 'required'
                }
            ...
            return validator.validate({test: 0});
        });
    });
