...
class Validator extends EventEmitter {
  ...
  test(filters, errors, messages, value, key) {
    ...
    for(i = 0; i < filters.length; i++) {
      ...
      const result = this.runFilter(name, args);

      if(this.isPromise(result)) {
        promises[i] = result.catch(error => {
  ...
  fail(name, messages, args, value, key, errors) {
  ...
  validate(data) {
    ...
    const promisesChain = _(rules).omit(ignore_keys).mapValues((filterData, key) => {
    ...
    .map((filterData, key) => {
      return this.validateRule(filterData, key, messages, errors, data, replaces);
    });

    const promises = promisesChain.value();
    const hasPromise = promises.some(promise => {
  ...
  updateErrors(data, errors, messages) {
    const errorsLength = _.size(errors);
  ...
  validateRule(filterData, key, messages, errors, data, replaces) {
    ...
    const result = this.test(filters, found, found_messages, value, transformed_key);
  ...
  updateValidateData(errors, messages, key, found, found_messages) {
    const foundLength = _.size(found);
